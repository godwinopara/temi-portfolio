...
const Article = () => {
  ...
  return <section className={styles.page}>
    ...
    {featuredArticle && <article className={styles.featured}>
      ...
      <div className={styles.featuredCopy}><p className="eyebrow">Featured / {featuredArticle.publication}</p><h2>{featuredArticle.title}</h2><p>{featuredArticle.excerpt}</p><a href={featuredArticle.link} target="_blank" rel="noreferrer">Read article ↗</a></div>
    </article>}
    <section className={styles.index} data-article-index aria-labelledby="all-writing-title"><div className={styles.indexTitle}><p className="eyebrow">The index</p><h2 id="all-writing-title">All writing</h2></div><ol>{articleRows.map((article, index) => <li key={article.id}><span>0{String(index + 1).padStart(2, "0")}</span><div><p>{article.publication}</p><h3><a href={article.link} target="_blank" rel="noreferrer">{article.title}</a></h3><p>{article.excerpt}</p></div><a className={styles.arrow} href={article.link} target="_blank" rel="noreferrer" aria-label={`Read ${article.title}`}>↗</a></li>)}</ol></section>
  </section>;
};
