...
export default function Home() {
  ...
  return <>
    ...
    <section id="work" className={styles.work} aria-labelledby="work-title"><div className={styles.sectionHeader}><div><p className="eyebrow">Selected work</p><h2 id="work-title">Ideas, made real.</h2></div><Link href="/projects">View all work ↗</Link></div><div className={styles.workGrid}>{selectedProjects.map((project, index) => <article className={`${styles.workCard} ${index === 1 ? styles.workCardOffset : ""}`} key={project.id}><div className={styles.workImage}><Image src={project.image} fill sizes="(min-width: 900px) 55vw, 100vw" alt={`${project.title} project artwork`} /><span>{project.category} / {project.year}</span></div><div className={styles.cardTitle}><h3>{project.title}</h3>{project.link ? <a href={project.link} target="_blank" rel="noreferrer" aria-label={`Open ${project.title}`}>↗</a> : <span aria-label="Currently unavailable">—</span>}</div></article>)}</div></section>
    <section id="speaking" className={styles.speaking} aria-labelledby="speaking-title"><div className={styles.sectionHeader}><div><p className="eyebrow">In the room</p><h2 id="speaking-title">Ideas in public.</h2></div><Link href="/talks">All talks ↗</Link></div><div className={styles.talkGrid}>{selectedTalks.map((talk) => <article className={styles.talkCard} key={talk.id}><div className={styles.talkImage}><Image src={talk.image} fill sizes="(min-width: 900px) 48vw, 100vw" alt={`${talk.title} at ${talk.host}`} /></div><p>{talk.host}</p><h3>{talk.title}</h3><a href={talk.link} target="_blank" rel="noreferrer">View talk ↗</a></article>)}</div></section>
    ...
    <section id="contact" className={styles.contact} aria-labelledby="contact-title"><p className="eyebrow">Speaking · press · collaborations</p><h2 id="contact-title">Let’s make the conversation <em>useful.</em></h2><a href={site.contactEmail}>{site.contactLabel} <span>↗</span></a></section>
  </>;
}
